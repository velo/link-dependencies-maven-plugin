...
public class CopyDependenciesMojo
    ...
    protected void doExecute()
        ...
        if (!useRepositoryLayout) {
            ...
            ProjectBuildingRequest buildingRequest = getRepositoryManager().setLocalRepositoryBasedir(session.getProjectBuildingRequest(),
                    outputDirectory);
...

    {
        for (Artifact artifact : artifacts) {
            ...
            // Copy the pom
            if (pomArtifact != null && pomArtifact.getFile() != null && pomArtifact.getFile().exists()) {
                File pomDestFile = new File(destDir, DependencyUtil.getFormattedFileName(pomArtifact, removeVersion, prependGroupId,
                        useBaseVersion, removeClassifier));
